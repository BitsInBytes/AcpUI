...
- **[Feature Doc] - Backend Architecture.md** — Compact backend orientation: server/runtime boundaries, provider isolation pillars, socket/ACP/MCP/persistence ownership, and handoffs to detailed feature docs. **Load this when starting any backend work.**
- **[Feature Doc] - Frontend Architecture.md** — Compact frontend orientation: app roots, socket/store/rendering pillars, identity routing, event ownership, and handoffs to detailed feature docs. **Load this when starting any frontend work.**
...
- **[Feature Doc] - Provider Status Panel.md** — Real-time status display for AI provider metrics (quota, spend, rate limits). Covers the backend cache/persistence mechanism, frontend routing pipeline, type contract, and component rendering (compact summary + details modal). Load this when implementing provider status emission, debugging status not appearing, or extending status metrics.
...
- **[Feature Doc] - Chat Header.md** — Top-level chat status and control bar. Covers provider/session title resolution, connection indicator rendering, pop-out mode control gating, and UI store toggles for sidebar/file explorer/help/system settings. Load this when implementing header controls, debugging missing/incorrect header titles, or investigating pop-out header behavior.
...
- **[Feature Doc] - Chat Input and Prompt Area.md** — Footer input component with file attachments, image compression, model quick-select, slash command autocomplete, and Scratch Pad entry control. Covers textarea auto-height, clipboard paste handling, HTTP file upload with multer, sharp image compression (JPEG quality 85), model selection UI, and context usage display. Load this when implementing input features, debugging attachments/uploads, or extending model selection.
- **[Feature Doc] - Notes.md** — Per-session Scratch Pad ownership across socket handlers, SQLite persistence, modal/store state, indicators, and pop-out availability. Load this when implementing `get_notes`/`save_notes`, notes UI behavior, notes indicators, or detached-window notes behavior.
...
- **[Feature Doc] - Help Docs Modal.md** — Read-only in-app Markdown documentation browser opened from the ChatHeader. Covers repository Markdown discovery, searchable file tree rendering, backend `help_docs_list`/`help_docs_read` handlers, and repo-root read validation. Load this when changing Help, documentation browsing, or repo-scoped Markdown read behavior.
