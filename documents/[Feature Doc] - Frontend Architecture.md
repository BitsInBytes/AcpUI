...
The hook registers turn-level socket listeners. Chat events route to `useStreamStore`; stats and rename events update `useSessionLifecycleStore`; shell events update `useShellRunStore`, patch timeline tool steps by `shellRunId`, and create a fallback shell tool step when shell lifecycle events arrive before a provider `tool_start`; sub-agent events update `useSubAgentStore` and lazily create sidebar `ChatSession` shells when a sub-agent first emits token or system-event output.
...
| Sub-agent panel | `frontend/src/components/SubAgentPanel.tsx` | `SubAgentPanel`, `handleStop`, `respond_permission` | Inline sub-agent status, Stop action, output, tools, and permission response UI |
...
The first `sub_agent_started` event stamps `invocationId` onto the active `ux_invoke_subagents` or `ux_invoke_counsel` tool step. `SubAgentPanel` uses that value so each historical tool step displays its own batch of agents, and `isInvocationActive` keeps the active orchestration step expanded until the invocation is terminal unless the user manually collapses it.
