...
describe('Provider Interop Hooks', () => {
  ...
  describe('acpClient.init() Handshake Hook', () => {
    it('should call performHandshake', async () => {
      ...
      vi.spyOn(providerLoader, 'getProviderModule').mockResolvedValue(mockModule);
      ...
      acpClient.providerModule = mockModule;
      acpClient.transport.sendRequest = vi.fn().mockResolvedValue({ result: 'initialized' });
  ...
  describe('acpUpdateHandler.handleUpdate() Normalization Hook', () => {
    ...
    it('should use provider.extractFilePath', async () => {
        ...
        expect(mockEmit).toHaveBeenCalledWith('system_event', expect.objectContaining({ filePath: '/resolved/path.txt' }));
    });
  });
  ...
  describe('acpCleanup.cleanupAcpSession() Discovery Hook', () => {
    it('should use provider.deleteSessionFiles', async () => {
      ...
      await acpCleanup.cleanupAcpSession('acp-123', 'test-provider');

      expect(mockDelete).toHaveBeenCalledWith('acp-123');
    });
  });
});
