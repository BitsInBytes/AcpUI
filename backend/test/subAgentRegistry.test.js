...
describe('subAgentRegistry', () => {
  ...
  it('registerSubAgent adds entry', () => {
    registerSubAgent('test-provider', 'sub-1', 'parent-1', 'do stuff', 'agent-dev');
    const entry = getSubAgent('sub-1');
    expect(entry).toEqual({ providerId: 'test-provider', parentAcpSessionId: 'parent-1', prompt: 'do stuff', agent: 'agent-dev', status: 'running' });
  });

  it('completeSubAgent sets status', () => {
    registerSubAgent('test-provider', 'sub-1', 'parent-1', 'p', 'a');
  ...
  it('getSubAgentsForParent filters correctly', () => {
    registerSubAgent('test-provider', 'sub-1', 'parent-1', 'p1', 'a');
    registerSubAgent('test-provider', 'sub-2', 'parent-2', 'p2', 'a');
    registerSubAgent('test-provider', 'sub-3', 'parent-1', 'p3', 'a');
  ...
  it('getAllRunning returns only running', () => {
    registerSubAgent('test-provider', 'sub-1', 'p', 'p1', 'a');
    registerSubAgent('test-provider', 'sub-2', 'p', 'p2', 'a');
    completeSubAgent('sub-1');
  ...
  it('removeSubAgentsForParent(null) clears all', () => {
    registerSubAgent('test-provider', 'sub-1', 'p1', 'p', 'a');
    registerSubAgent('test-provider', 'sub-2', 'p2', 'p', 'a');
  ...
  it('removeSubAgentsForParent(id) removes only matching', () => {
    registerSubAgent('test-provider', 'sub-1', 'parent-1', 'p', 'a');
    registerSubAgent('test-provider', 'sub-2', 'parent-2', 'p', 'a');
