...
export default function registerArchiveHandlers(io, socket) {
  ...
  socket.on('delete_archive', ({ folderName }, callback) => {
    try {
      const provider = getProvider();
      const archivePath = provider.config.paths.archive;
  ...
  socket.on('list_archives', (callback) => {
    try {
      const provider = getProvider();
      const archivePath = provider.config.paths.archive;
      if (!archivePath || !fs.existsSync(archivePath)) return callback({ archives: [] });
  ...
  socket.on('restore_archive', async ({ folderName }, callback) => {
    try {
      const provider = getProvider();
      const archivePath = provider.config.paths.archive;
