...
const FileExplorer: React.FC = () => {
  ...
  const loadDir = useCallback((dirPath: string, cb: (items: DirEntry[]) => void) => {
    socket?.emit('explorer_list', { dirPath }, (res: { items?: DirEntry[] }) => {
      cb(res.items || []);
    });
  }, [socket]);
  ...
  useEffect(() => {
    ...
    socket.emit('explorer_root', (res: { root?: string }) => {
  ...
  const openFileHandler = (filePath: string) => {
    socket?.emit('explorer_read', { filePath }, (res: { content?: string }) => {
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    socket.emit('explorer_write', { filePath: openFile.path, content: openFile.content }, () => {
  ...
  const handleChange = (content: string) => {
    ...
    saveTimer.current = setTimeout(() => {
      ...
      socket.emit('explorer_write', { filePath: openFile.path, content });
