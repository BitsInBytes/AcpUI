...
const getFilePathFromEvent = (event: SystemEvent): string | undefined => {
  ...
  if (event.toolName === 'run_shell_command') return undefined;
  if (event.toolName === 'invoke_sub_agents') return undefined;
      ...
      <AnimatePresence>
        {!isCollapsed && (
          ...
          >
            ...
            {(step.event.toolName === 'invoke_sub_agents' || step.event.toolName === 'counsel') && (
              <SubAgentPanel />
            )}
          </motion.div>
        )}
