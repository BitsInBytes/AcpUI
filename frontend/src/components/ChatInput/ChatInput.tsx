...
const ChatInput: React.FC = () => {
  ...
  const slashCommands = useSystemStore(state => state.slashCommands);
  ...
  const handleMergeFork = () => {
    ...
    setMerging(true);
    // Capture IDs now — activeSession may be stale inside the async callback closure
    ...
    socket.emit('merge_fork', { uiId: forkId }, (res: { success?: boolean; parentUiId?: string; error?: string }) => {
      setMerging(false);
      if (res.success) {
        // Switch to parent before removing the fork — removing first would leave
        // activeSessionId pointing at a deleted session
        useChatStore.getState().handleSessionSelect(socket, parentId);
        setTimeout(() => {
  ...
  const selectSlashCommand = (cmd: typeof slashCommands[0]) => {
    ...
    if (!hasArgs) {
      // Simple command — send immediately
    ...
    <footer className="input-container">
      ...
      <div className={`input-wrapper ${activeSession?.isWarmingUp ? 'warming-up' : ''}`}>
        ...
        >
          ...
          <button
            type="button"
            className={`input-action-btn ${useChatStore.getState().sessionNotes[activeSessionId || ''] ? 'has-notes' : ''}`}
          ...
          <div className="textarea-container">
            ...
            <textarea
              ...
              placeholder={activeSession?.isTyping ? useSystemStore.getState().branding.busyText : activeSession?.isHooksRunning ? (useSystemStore.getState().branding.hooksText || '⚙ Cleaning up...') : !isEngineReady ? (useSystemStore.getState().branding.warmingUpText || 'Engine warming up...') : activeSession?.isWarmingUp ? (useSystemStore.getState().branding.resumingText || 'Resuming...') : (useSystemStore.getState().branding.inputPlaceholder || 'Send a message...')}
              disabled={isDisabled}
              rows={1}
            />
          </div>
