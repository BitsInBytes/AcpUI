...
const NotesModal: React.FC = () => {
  ...
  const saveNotes = useCallback((text: string) => {
    ...
    useSessionLifecycleStore.setState(state => ({ sessionNotes: { ...state.sessionNotes, [activeSessionId]: text.length > 0 } }));
        ...
        >
          ...
          <div className="modal-body notes-body">
            {activeTab === 'raw' ? (
              ...
              <div className="notes-rendered message-wrapper markdown-body">
                {notes ? (
                  <ReactMarkdown
