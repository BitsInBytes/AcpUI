...
const AssistantMessage: React.FC<AssistantMessageProps> = ({
  ...
  const isHooksRunning = useChatStore(state => state.sessions.find(s => s.acpSessionId === acpSessionId)?.isHooksRunning);
      ...
      <div className="message">
        <div className="message-role-wrapper">
          ...
          {!isArchived && !isStreaming && (
            <div style={{ display: 'flex', gap: '2px' }}>
              ...
              {!useChatStore.getState().sessions.find(s => s.id === activeSessionId)?.isSubAgent && (
        ...
        <div className="unified-timeline">
          {timeline && timeline.map((step, index) => {
            const isCollapsed = localCollapsed[index];

            if (step.type === 'text') {
              return (
                <div key={index} className="message-content response-tab-content">
        ...
        {content && (!timeline || !timeline.some(s => s.type === 'text')) && (
          <div className="message-content response-tab-content fallback-content">
