...
describe('useFileUpload', () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    useInputStore.setState({ attachmentsMap: { [activeSessionId]: [] },
      setAttachments: vi.fn((sessionId, updater) => {
        const current = useInputStore.getState().attachmentsMap[sessionId] || [];
        const next = typeof updater === 'function' ? updater(current) : updater;
        useInputStore.setState(state => ({ attachmentsMap: { ...state.attachmentsMap, [sessionId]: next } }));
      })
    });
  ...
  it('initializes with attachments from store', () => {
    ...
    useInputStore.setState({ attachmentsMap: { [activeSessionId]: [attachment] } });
  ...
  it('handleFileUpload uploads files and updates store', async () => {
    ...
    expect(useInputStore.getState().attachmentsMap[activeSessionId].length).toBe(1);
  });
