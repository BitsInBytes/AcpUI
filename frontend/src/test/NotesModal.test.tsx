...
describe('NotesModal', () => {
  ...
  beforeEach(() => {
    ...
    act(() => {
      ...
      useSessionLifecycleStore.setState({ activeSessionId: 's1', sessionNotes: {} });
  ...
  it('updates sessionNotes on save', () => {
    ...
    act(() => { vi.advanceTimersByTime(600); });
    expect(useSessionLifecycleStore.getState().sessionNotes['s1']).toBe(true);
  });

  it('sets sessionNotes to false when cleared', () => {
    act(() => { useSessionLifecycleStore.setState({ sessionNotes: { s1: true } }); });
    render(<NotesModal />);
    const textarea = screen.getByPlaceholderText(/Write notes here/);
    fireEvent.change(textarea, { target: { value: '' } });
    act(() => { vi.advanceTimersByTime(600); });
    expect(useSessionLifecycleStore.getState().sessionNotes['s1']).toBe(false);
  });
