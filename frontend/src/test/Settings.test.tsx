...
describe('App Settings Modal', () => {
  beforeEach(() => {
    ...
    act(() => {
      ...
      useSessionLifecycleStore.setState({ sessions: [{ id: 's1', name: 'Chat 1', messages: [], model: 'test-balanced', isTyping: false, isWarmingUp: false, acpSessionId: 'acp-1' }],
        activeSessionId: 's1'
      });
  ...
  it('handles model selection change', async () => {
    ...
    await act(async () => {
      useSessionLifecycleStore.setState({ handleSessionModelChange });
      useUIStore.setState({ isSettingsOpen: true, settingsSessionId: 's1' });
    });
    ...
    fireEvent.change(select, { target: { value: 'test-flagship' } });

    expect(handleSessionModelChange).toHaveBeenCalledWith(expect.anything(), 's1', 'test-flagship');
  });
  ...
  it('handles session deletion with confirmation', async () => {
    ...
    await act(async () => {
      useSessionLifecycleStore.setState({ handleDeleteSession });
