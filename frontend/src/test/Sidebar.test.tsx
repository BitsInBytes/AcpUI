...
describe('Sidebar', () => {
  ...
  it('applies typing class when a session is typing', () => {
    act(() => {
      useChatStore.setState({
        sessions: [{ ...mockSessions[0], isTyping: true }, mockSessions[1]],
      });
    });
    render(<Sidebar />);
  ...
  it('applies unread class when a session has unread response', () => {
    act(() => {
      useChatStore.setState({
        sessions: [{ ...mockSessions[0], hasUnreadResponse: true }, mockSessions[1]],
      });
    });
    render(<Sidebar />);
    const sessionOne = screen.getByText('Chat One').closest('.session-item');
  ...
  it('applies awaiting-permission class when a session is awaiting permission', () => {
    act(() => {
      useChatStore.setState({
        sessions: [{ ...mockSessions[0], isAwaitingPermission: true }, mockSessions[1]],
      });
    });
    render(<Sidebar />);
    const sessionOne = screen.getByText('Chat One').closest('.session-item');
  ...
  it('calls handleTogglePin when pin button is clicked', () => {
    ...
    act(() => {
      useChatStore.setState({ handleTogglePin });
    });
    render(<Sidebar />);
  ...
  it('saves new name on Enter key', () => {
    ...
    act(() => {
      useChatStore.setState({ handleRenameSession });
    });
    render(<Sidebar />);
    const sessionOne = screen.getByText('Chat One');
    fireEvent.contextMenu(sessionOne);
  ...
  it('renders workspace CWD buttons when workspaceCwds is set', () => {
    act(() => {
  ...
  it('archive button opens archive modal', () => {
    const emitMock = vi.fn((_event: string, cb: any) => {
      cb({ archives: ['archived-chat-1', 'archived-chat-2'] });
    });
    act(() => {
    ...
    expect(emitMock).toHaveBeenCalledWith('list_archives', expect.any(Function));
    expect(screen.getByText('Archived Chats')).toBeInTheDocument();
  });

  it('archive modal shows list of archived chats', () => {
    const emitMock = vi.fn((_event: string, cb: any) => {
      cb({ archives: ['my-old-chat', 'another-archive'] });
    });
    act(() => {
      useSystemStore.setState({ socket: { emit: emitMock } as any });
    });
  ...
  it('clicking archive item calls restore_archive socket emit', () => {
    const emitMock = vi.fn((event: string, ...args: any[]) => {
      if (event === 'list_archives') args[0]({ archives: ['old-chat'] });
    ...
    fireEvent.click(screen.getByTitle('Restore archived chat'));
    fireEvent.click(screen.getByText('old-chat'));
    expect(emitMock).toHaveBeenCalledWith('restore_archive', { folderName: 'old-chat' }, expect.any(Function));
  });
  ...
  it('Tab in search selects first filtered session', () => {
    ...
    act(() => {
      useChatStore.setState({ handleSessionSelect });
    });
    render(<Sidebar />);
    const searchInput = screen.getByPlaceholderText('Search chats...');
    fireEvent.change(searchInput, { target: { value: 'One' } });
  ...
  it('clicking workspace CWD button calls handleNewChat with cwd and agent', () => {
    const handleNewChat = vi.fn();
    act(() => {
      useChatStore.setState({ handleNewChat });
      useSystemStore.setState({
        workspaceCwds: [
          { label: '+ CE', path: '/repos/demo-project', agent: 'agent-dev', pinned: true },
        ],
...
describe('Sidebar - additional coverage', () => {
  ...
  it('workspace buttons each call handleNewChat with correct cwd and agent', () => {
    const handleNewChat = vi.fn();
    act(() => {
      useChatStore.setState({ handleNewChat });
      useSystemStore.setState({
  ...
  it('new folder button calls createFolder', () => {
  ...
  it('archive session removes it from the list', () => {
    ...
    expect(emitMock).toHaveBeenCalledWith('archive_session', { uiId: '1' });
  });
  ...
  it('delete archive removes it from the list', () => {
    const emitMock = vi.fn((event: string, ...args: any[]) => {
      if (event === 'list_archives') args[0]({ archives: ['arch1', 'arch2'] });
...
describe('Sidebar - workspace picker', () => {
  beforeEach(() => {
    ...
    act(() => {
      ...
      useChatStore.setState({
        sessions: [{ id: '1', acpSessionId: 'acp-1', name: 'Chat One', messages: [], isTyping: false, isWarmingUp: false, model: 'balanced', isPinned: false }],
        activeSessionId: '1',
        handleTogglePin: vi.fn(),
        handleRenameSession: vi.fn(),
        handleSessionSelect: vi.fn(),
  ...
  it('overflow + button appears when non-pinned workspaces exist', () => {
    render(<Sidebar />);
    expect(screen.getByTitle('More workspaces')).toBeInTheDocument();
  });

  it('overflow + button does NOT appear when all workspaces are pinned', () => {
    act(() => {
      useSystemStore.setState({
        workspaceCwds: [
          { label: '+ CE', path: '/repos/demo-project', agent: 'agent-dev', pinned: true },
  ...
  it('clicking overflow + button opens workspace picker modal', () => {
    render(<Sidebar />);
    fireEvent.click(screen.getByTitle('More workspaces'));
...
describe('Sidebar - utility row and workspace row layout', () => {
  ...
  it('sidebar header renders workspace-row and utility-row as separate divs', () => {
    ...
    const workspaceRow = headerActions.querySelector('.sidebar-workspace-row');
    const utilityRow = headerActions.querySelector('.sidebar-utility-row');
  ...
  it('pinned workspace labels appear in buttons instead of New Chat', () => {
    act(() => {
      useSystemStore.setState({
        workspaceCwds: [
          { label: '+ CE', path: '/repos/demo-project', agent: 'agent-dev', pinned: true },
    ...
    expect(workspaceRow.textContent).toContain('+ CE');
    expect(workspaceRow.textContent).toContain('+ TS');
    expect(workspaceRow.textContent).not.toContain('New Chat');
  });
});
