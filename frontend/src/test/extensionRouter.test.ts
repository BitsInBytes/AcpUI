...
describe('routeExtension', () => {
  ...
  it('routes generic provider status', () => {
    const status = {
      ...
      sections: [{
        ...
        items: [{ id: 'status-item', label: 'Metric', value: '12 units', progress: { value: 0.12 } }]
      }]
    };
    const result = routeExtension('ext/provider/status', { status }, PREFIX, sysCmds, customCmds);
    expect(result).toEqual({ type: 'provider_status', status });
  });

  it('routes generic provider_status alias', () => {
    const status = { sections: [{ id: 'section', items: [] }] };
    const result = routeExtension('ext/provider_status', { status }, PREFIX, sysCmds, customCmds);
    expect(result).toEqual({ type: 'provider_status', status });
  });

  it('ignores malformed provider status payloads', () => {
    expect(routeExtension('ext/provider/status', { status: null }, PREFIX, sysCmds, customCmds)).toBeNull();
    expect(routeExtension('ext/provider/status', { status: { providerId: 'provider-id' } }, PREFIX, sysCmds, customCmds)).toBeNull();
  });
