...
function PopOutApp() {
  ...
  useEffect(() => {
    ...
    useChatStore.setState({ activeSessionId: popoutSessionId });
    ...
    socket.emit('load_sessions', (res: { sessions?: ChatSession[] }) => {
      if (res.sessions) {
        ...
        useChatStore.setState({ sessions: mapped, activeSessionId: popoutSessionId });

        // Hydrate the session
        const session = mapped.find((s: ChatSession) => s.id === popoutSessionId);
        if (session?.acpSessionId) {
          socket.emit('watch_session', { sessionId: session.acpSessionId });
          useChatStore.getState().hydrateSession(socket, popoutSessionId);
        }
        setReady(true);
      }
    });
