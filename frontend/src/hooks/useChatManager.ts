...
export function useChatManager(
  ...
  useEffect(() => {
    ...
    let toolOutputMaxLines: number | null = null;
      ...
      useSessionLifecycleStore.setState(state => ({ sessions: state.sessions.map(s => {
          ...
          const timeline = msg.timeline.map(entry =>
            entry.type === 'tool' && entry.event.status === 'in_progress'
              ? { ...entry, event: { ...entry.event, output: trimShellOutputLines((entry.event.output || '') + chunk, toolOutputMaxLines) } }
              : entry
          );
    ...
    socket.on('tool_output_stream', (data: { chunk: string; maxLines?: number }) => {
      ...
      toolOutputBuffer = trimShellOutputLines(toolOutputBuffer, toolOutputMaxLines);
    ...
    // Sub-agent events
    socket.on('sub_agent_started', (data: { providerId: string; acpSessionId: string; uiId: string; parentUiId: string | null; index: number; name: string; prompt: string; agent: string; model?: string }) => {
      ...
      if (data.index === 0) {
        ...
        const oldSubAgents = useSessionLifecycleStore.getState().sessions.filter(s => s.isSubAgent && s.forkedFrom === parentUiId);
        for (const old of oldSubAgents) {
          socket.emit('delete_session', { uiId: old.id });
        }
        useSessionLifecycleStore.setState(state => ({ sessions: state.sessions.filter(s => !(s.isSubAgent && s.forkedFrom === parentUiId)) }));
      }
