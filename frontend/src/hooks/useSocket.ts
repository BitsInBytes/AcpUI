...
function getOrCreateSocket(): Socket {
  ...
  _socket.on('provider_extension', (data: { method: string; params: any }) => {
    ...
    const p = data.params || {};
    const ext = useSystemStore.getState().branding.protocolPrefix || '_provider/';
    const result = routeExtension(data.method, p, ext, [], useSystemStore.getState().customCommands);
    if (!result) return;

    if (result.type === 'commands') {
      useSystemStore.getState().setSlashCommands(result.commands);
      ...
      useChatStore.setState(state => ({
        sessions: state.sessions.map(s => s.acpSessionId === result.sessionId ? { ...s, isTyping: true } : s)
      }));
      ...
      const waitAndSave = () => {
        ...
        useChatStore.setState(state => ({
          sessions: state.sessions.map(s => s.acpSessionId === result.sessionId ? { ...s, isTyping: false } : s)
        }));
        const session = useChatStore.getState().sessions.find(s => s.acpSessionId === result.sessionId);
