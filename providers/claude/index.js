...
export function parseExtension(method, params) {
  ...
  switch (type) {
    ...
    case 'compaction/status':
      ...
      break;
    case 'quota_update':
      result = { type: 'quota_update', sessionId: params.sessionId, quotaData: params.quotaData };
      break;
    case 'config_options':
...
export function restoreSessionFiles(savedAcpId, archiveDir) {
  ...
  if (fs.existsSync(jsonlSrc)) {
    fs.copyFileSync(jsonlSrc, path.join(sessionsRoot, `${savedAcpId}.jsonl`));
  }
  if (fs.existsSync(jsonSrc)) {
    fs.copyFileSync(jsonSrc, path.join(sessionsRoot, `${savedAcpId}.json`));
  }
...
export async function setInitialAgent(acpClient, sessionId, agent) {
  // console.log('setInitialAgent called', { sessionId, agent });
  return;
}
