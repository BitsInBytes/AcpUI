...
export async function setInitialAgent(acpClient, sessionId, agent) {
  ...
  const sendWithTimeout = (method, params, timeout = 30000) => {
    return Promise.race([
      acpClient.sendRequest(method, params),
  ...
  await acpClient.waitForDrainToFinish(sessionId, 10000);
...
export function normalizeTool(event, update) {
  ...
  // Use toolIdPattern to extract the real tool name from prefixed IDs
  const { config } = getProvider();
  const prefix = (config.toolIdPattern && config.mcpName)
    ? config.toolIdPattern.replace('{mcpName}', config.mcpName).replace('{toolName}', '')
    : null;

  if (prefix) {
    // Strip prefix from toolName (e.g. "@AcpUI/run_shell_command" → "run_shell_command")
    if (toolName.startsWith(prefix)) {
      toolName = toolName.slice(prefix.length);
    }
    // If toolName is still a generic ID, extract from title (Kiro often omits `name`)
    if (toolName.startsWith('tooluse_') || toolName.startsWith('call_') || toolName.startsWith('toolu_')) {
      const title = event.title || '';
      const prefixInTitle = title.indexOf(prefix);
      if (prefixInTitle !== -1) {
        toolName = title.slice(prefixInTitle + prefix.length).split(/[\s:,]/)[0];
      }
    }
    // Clean the prefix from the display title
    if (event.title) {
      event = { ...event, title: event.title.replaceAll(prefix, '') };
    }
  }
  ...
  // Format title: "Running: invoke_sub_agents" → "Invoke Sub Agents"
