...
describe('Codex Provider', () => {
  ...
  describe('tool helpers', () => {
    ...
    it('normalizes Codex MCP tool titles and categorizes UI shell calls', () => {
      ...
      const normalizeCore = (tool, args = {}) => codex.normalizeTool(
        { type: 'tool_start', id: `t-${tool}`, title: `Tool: AcpUI/${tool}` },
        { title: `Tool: AcpUI/${tool}`, rawInput: { invocation: { server: 'AcpUI', tool, arguments: args } } }
      );
      expect(normalizeCore('ux_invoke_subagents').title).toBe('Run Subagents');
      expect(normalizeCore('ux_check_subagents').title).toBe('Check Subagents: Waiting for agents to finish');
      expect(normalizeCore('ux_check_subagents', { waitForCompletion: false }).title).toBe('Check Subagents: Quick status check');
